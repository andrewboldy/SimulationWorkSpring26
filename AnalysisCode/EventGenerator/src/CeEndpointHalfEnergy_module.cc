...
namespace mu2e {
    ...
    , pdgId_(conf().pdgId())
  {
    ...
    if (pid == PDGCode::e_minus) {
      ...
      //endPointEnergy_ = GlobalConstantsHandle<PhysicsParams>()->getEndpointEnergy(conf().stoppingTargetMaterial()); //using original the endpoint energy to see results
      endPointEnergy_ = GlobalConstantsHandle<PhysicsParams>()->getEndPointEnergy(conf().stoppingTargetMaterial())/2; //changing to half the endpoint energy
    }
  ...
  void CeEndpoint::produce(art::Event& event) {
    ...
    CLHEP::HepLorentzVector fourMom = CLHEP{randomUnitSphere_.fire(endPointMomentum_), endPointEenrgy_};
