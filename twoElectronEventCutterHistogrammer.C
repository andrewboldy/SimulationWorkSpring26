...
void twoElectronEventCutterHistogrammer(const string& generatorName, const string& fileName)
{
  ...
  // are below that scale for this study.  The Z range is much wider than X/Y
  // because the detector and production geometry extend farther along z.
  ...
  //   output: angle from the z-axis in degrees
  //
  // The clamp to [-1, 1] protects acos from tiny floating-point roundoff errors.
  ...
  for (int i_event = 0; i_event < numEvents; i_event++)
  {
      ...
      if (track.trkmcsim != nullptr)
      {
        ...
        for (const auto& mctrack : *(track.trkmcsim))
        {
          ...
          if (mctrack.rank == 0)
          {
            ...
            // Compute and store this electron's angle from the z-axis for the text file.
    ...
    else
    {
      ...
      // The two angle values are safe to access with .at(0) and .at(1) because
  ...
  //----------------------------------------------------------------------------
  // PDF 3: selected-event reconstructed trksegs momentum overlay
  //
  ...
  //----------------------------------------------------------------------------
  // PDF 4: reconstructed trksegs event-selection comparison
  //
  ...
  //----------------------------------------------------------------------------
  // PDF 5: selected-event trkmcsim origin 1D histograms
  //
  // Six pads:
  ...
  //----------------------------------------------------------------------------
  // PDF 6: selected-event trkmcsim origin 2D maps
  //
  // Six pads:
