...
namespace mu2e {
  ...
  //================================================================
  class CeEndpointDiffEnergy : public art::EDProducer {
  public:
    struct Config {
    ...
    explicit CeEndpointDiffEnergy(const Parameters& conf);
  ...
  //================================================================
  CeEndpointDiffEnergy::CeEndpointDiffEnergy(const Parameters& conf)
    : EDProducer{conf}
    ...
    , pdgId_(conf().pdgId())
  {
    ...
    if (pid == PDGCode::e_minus) {
      process = ProcessCode::mu2eCeMinusEndpointDiffEnergy;
      ...
      double testEnergy_ = endPointEnergy_/2;
    }
    else {
    ...
    testMomentum_ = testEnergy_*sqrt(1-std::pow(electronMass_/testEnergy_,2));
    if(verbosity_ > 0) {
      mf::LogInfo log("CeEndpointDiffEnergy");
      log<<"stoppingTargetMaterial = "<<conf().stoppingTargetMaterial()
         <<", endpoint energy = "<<testEnergy_
  ...
  void CeEndpoint::produce(art::Event& event) {
    ...
    output->emplace_back(mustop,
                         ...
                         CLHEP::HepLorentzVector{randomUnitSphere_,fire(testMomentum_),testEnergy_},
...
DEFINE_ART_MODULE(mu2e::CeEndpointDiffEnergy)
